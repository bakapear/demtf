...
(function (root) {
	...
	BitStream.prototype.readBitVar = function (signed) {
		switch (this.readBits(2)) {
			case 0: return this.readBits(4, signed);
			case 1: return this.readBits(8, signed);
			case 2: return this.readBits(12, signed);
			case 3: return this.readBits(32, signed);
		}
		throw new Error('Invalid var bit');
	}

	BitStream.prototype.writeBitVar = function (value, signed) {
		const bitsNeeded = (signed) ? Math.floor(Math.log2(Math.abs(value))) + 2 : Math.floor(Math.log2(value)) + 1;
		if (signed) {
	...
	BitStream.prototype.readVarInt = function (signed) {
		let result = 0;
	...
	BitStream.prototype.writeVarInt = function (value, signed) {
		value = makeUnsigned(value, signed);
	
		do {
			const byte = value & 0x7F;
			const resumeBit = (value >= 128) ? 0x80 : 0;
			this.writeUint8(byte | resumeBit);
			value = value >> 7;
		} while (value > 0);
	}
	...
	class DynamicBitStream extends BitStream {
	...
	if (typeof define !== 'undefined' && define.amd) {
		define(function () {
			return {
				BitView: BitView,
				BitStream: BitStream,
				DynamicBitView: DynamicBitView,
				DynamicBitStream: DynamicBitStream
			};
		});
	}
	// Node.js
